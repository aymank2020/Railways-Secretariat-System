...
class DatabaseService {
  ...
  Future<Database> _initDatabase() async {
    ...
    if (kIsWeb) {
      ...
      path = join(documentsDirectory.path, 'secretariat.db');
    }
  ...
  Future<int> insertUser(UserModel user) async {
    final db = await database;
    return _insertUserInternal(db, user);
  }
  ...
  Future<int> updateWarid(WaridModel warid, int userId, String userName) async {
    ...
    final id = await db.update(
      'warid',
      {...waridToSave.toMap(), 'updated_at': DateTime.now().toIso8601String()},
      where: 'id = ?',
      whereArgs: [warid.id],
    );
  ...
  Future<List<WaridModel>> getAllWarid(
    ...
    if (search != null && search.isNotEmpty) {
      whereClause =
        ...
        OR letter_number LIKE ? OR recipient_1_name LIKE ? OR recipient_2_name LIKE ?
        OR recipient_3_name LIKE ? OR file_name LIKE ?)''';
      final searchPattern = '%$search%';
      whereArgs = List.filled(8, searchPattern);
    }
  ...
  Future<int> updateSadir(SadirModel sadir, int userId, String userName) async {
    ...
    final id = await db.update(
      'sadir',
      {...sadirToSave.toMap(), 'updated_at': DateTime.now().toIso8601String()},
      where: 'id = ?',
      whereArgs: [sadir.id],
    );
  ...
  Future<List<SadirModel>> getAllSadir(
    ...
    if (search != null && search.isNotEmpty) {
      whereClause =
        ...
        OR letter_number LIKE ? OR sent_to_1_name LIKE ? OR sent_to_2_name LIKE ?
        OR sent_to_3_name LIKE ? OR file_name LIKE ?)''';
      final searchPattern = '%$search%';
      whereArgs = List.filled(8, searchPattern);
    }
