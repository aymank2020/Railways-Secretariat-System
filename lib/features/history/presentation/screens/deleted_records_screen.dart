...
class _DeletedRecordsScreenState extends State<DeletedRecordsScreen> {
  ...
  void _loadMoreDeletedRecords() {
    final provider = Provider.of<DocumentProvider>(context, listen: false);
    provider.loadMoreDeletedRecords();
  }
  ...
  WaridModel _buildWaridFromDeleted(DeletedRecordModel record) {
    ...
    return WaridModel(
      ...
      chairmanIncomingNumber:
          _toNullableText(payload['chairman_incoming_number']),
      chairmanIncomingDate: _parseDate(payload['chairman_incoming_date']),
      chairmanReturnNumber: _toNullableText(payload['chairman_return_number']),
      chairmanReturnDate: _parseDate(payload['chairman_return_date']),
  ...
  Widget _buildLoadMoreFooter(DocumentProvider provider) {
    if (!provider.hasMoreDeletedRecords &&
        !provider.isLoadingMoreDeletedRecords) {
    ...
    return Padding(
      ...
      child: Align(
        child: OutlinedButton.icon(
          onPressed: provider.isLoadingMoreDeletedRecords
              ? null
              : _loadMoreDeletedRecords,
          icon: provider.isLoadingMoreDeletedRecords
              ? const SizedBox(
                  width: 16,
                  height: 16,
                    ...
                    : isCompact
                        ? Column(
                            children: [
                              ...
                              _buildLoadMoreFooter(documentProvider),
                            ],
                          )
                        : Column(
                            children: [
                              Expanded(child: _buildWideTable(records)),
                              _buildLoadMoreFooter(documentProvider),
                            ],
                          ),
          ),
